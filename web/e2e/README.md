...
- **One spec per story**, named `epic-<slug>-story-<N>-<title>.spec.ts`.
  - Example: `epic-application-shell-story-2-app-navigation.spec.ts`.
  - The epic segment is the epic's **slug**, not its number: the epic-end batched run
    works out which story a failure belongs to by parsing that name, so a numeric epic
    segment breaks per-story attribution. It's rejected at write time.
...
npm run test:e2e -- e2e/epic-application-shell-story-2-*.spec.ts   # run one story's spec
