...
- **Keep user-facing language simple and actionable.** Anything an end user
  ...
  docs, CHANGELOG entries — must be plain, direct, and free of dev-speak or
  unexplained jargon. Write for someone who isn't a developer.
...
- **Template maintenance** — editing `.claude/`, `.github/`, `.template-docs/`,
  ...
  [workflow-guard.js](.claude/hooks/workflow-guard.js) hook detects this repo
...
**Writing the notes.** Add a [CHANGELOG.md](CHANGELOG.md) `[Unreleased]` entry as each
change lands — `/release` copies them verbatim into the GitHub Release, where users read
them. Two or three sentences: what changed, and whether they need to do anything. Leave out
internal mechanism and why the approach was chosen, and merge several changes that tell one
story into a single entry rather than one per commit.
