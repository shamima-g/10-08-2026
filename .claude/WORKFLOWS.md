...
**Starting from a design?** Whatever you've designed — a Claude Design or Figma export, wireframes, or a hand-written HTML mockup — there's no separate import step: just drop the files into `documentation/` (any shape; loose files are fine too). At INTAKE, `/start` reads whatever design it finds, works out your screens (layout, fields, copy, colours), and reads it back for confirmation — including anything it couldn't determine — before building each epic faithfully to those screens. See [documentation/README.md](../documentation/README.md#working-from-an-existing-design) for how it reads your design. Later, when your design changes, update the files in `documentation/` and start a normal epic describing the change ("rebuild these screens to match my updated design") — a design update is ordinary, user-steered work, not a special mode.
...
**Two reports, one per audience.** Pick by who's reading:

| Command | Use it when | Page |
|---|---|---|
| `/build-report-maintainer` | You want the internal picture: how long it took, what it cost, how efficiently it ran, what each kind of screen costs to build, and where time was lost | `generated-docs/reports/build-report.html` |
...
**There are only two, and the split is audience — not subject.** The maintainer page carries everything internal in one place: effort and cost, workflow performance, what each kind of screen costs to build with a calculator for sizing the next feature, and — in a **Spend detail** fold — the per-model, per-tool and per-sub-agent breakdowns plus the verbatim log of every decision you were asked for. Those are all the same reader at different zoom levels, so they're sections, not separate commands. The client page exists separately because it works by *withholding* cost and internal machinery, which can't safely be a section you collapse.

Both are read-only and never touch workflow state, so run either at any time, during a build or after. Each report's written **“What this means”** panel is generated by following a brief inside that report's own skill file, which also documents what the report contains — edit the brief to change the report's focus, length or tone, bearing in mind that `/upgrade` replaces those skill files, so such edits last until the next template update. The steps they share live in [build-report-procedure.md](shared/build-report-procedure.md).

`/build-report-maintainer` refreshes the cost and effort data itself as its first step, so you don't run anything before it.
...
| `/build-report-maintainer` | Retrospective after (or during) a build; sizing the next piece of work | Effort and cost, workflow performance, what each kind of screen costs to build plus a sizing calculator, spend detail per model/tool/helper, every decision you were asked for, codebase stats, per-epic breakdown, stumbling blocks — as a visual page |
