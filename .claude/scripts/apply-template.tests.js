...
describe('applyTemplate (integration)', () => {
  ...
  // workflow-guard.js's dev-repo detection and strips the publish pipeline's exclusion list,
  ...
  // that most need it deleted. Simplifying this to workflow-guard.js's single-file check would
...
describe('allowlist invariants', () => {
  ...
  test('every prune-exempt dir is inside an owned tree (otherwise the exemption is a no-op)', () => {
    ...
    assert.ok(isPruneExempt('.claude/hooks/workflow-guard.js'), 'files inside an exempt dir are exempt');
    assert.ok(!isPruneExempt('.claude/hooks-extra/x.js'), 'exemption is dir-scoped, not a name prefix');
  });
