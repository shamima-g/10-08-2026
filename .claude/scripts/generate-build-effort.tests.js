#!/usr/bin/env node
/**
 * Tests for generate-build-effort.mjs — the effort-benchmark data generator.
 *
 ...
 *
 * Usage:
 *   node .claude/scripts/generate-build-effort.tests.js
 */
'use strict';
...
const { test, assert, assertEqual, assertDeepEqual, summary } = require('./lib/test-harness');
...
function run(root, extraArgs = []) {
  ...
  // No `html` here on purpose: this generator is DATA ONLY. The effort content is rendered as a
  // section of the maintainer build report from this JSON, so the page is asserted over there
  // (generate-build-report-html.tests.js). Anything this suite needs must be visible in the data.
  return {
    status, stdout, stderr,
    data: fs.existsSync(dataFile) ? JSON.parse(fs.readFileSync(dataFile, 'utf8')) : null,
    htmlFile: path.join(root, 'generated-docs', 'reports', 'build-effort.html'),
  };
}
...
tmp('the data file names each story and its screen type, and no page is written', (root) => {
  writeProject(root, { billing: { stories: story(1, 'invoice-table', 0, 30) } });
  const { data, htmlFile } = run(root);
  assertEqual(data.stories[0].title, 'invoice table', 'story title recorded');
  assertEqual(data.stories[0].cat, 'Listing / table page', 'screen type classified');
  assertEqual(data.categories[0].cat, 'Listing / table page', 'screen type reaches the benchmark table');
  // The standalone page is deliberately gone — the maintainer report renders this data instead.
  // Re-adding a page here would put the same tables in two renderers, which is what we removed.
  assert(!fs.existsSync(htmlFile), 'no standalone build-effort.html is produced');
});
...
tmp('cost below 60% story coverage degrades the whole report to time-only', (root) => {
  ...
  // `costComplete: false` is the whole contract with the renderer — it is what makes the
  // maintainer report drop every cost column instead of publishing unsupported figures.
  // The rendering half of that rule is asserted in generate-build-report-html.tests.js.
});
...
// Running the maintainer report IS a post-delivery session, and its spend lands outside every
// story window — so counting it here would raise `overheadCost`, raise `costUplift`, and push
// every "fully loaded" figure and the sizing calculator up a little more on each run. The cost
// generator already identifies these sessions (it has the transcript-hygiene walk needed to find
// each session's first deliberate input) and publishes the list; this reads it so both halves of
// the maintainer page count the same sessions.
tmp('post-delivery reporting sessions flagged by the cost generator are excluded too', (root) => {
  writeProject(root, { billing: { stories: story(1, 'invoice-table', 0, 30) } });
  const store = writeTranscripts(root, {
    build: { main: [], subagents: { developer: [usageLine({ id: 'b', ts: '2026-07-01T09:05:00Z' })] } },
    // Outside the story window, exactly where a report run's spend falls.
    reporting: { main: [], subagents: { developer: [usageLine({ id: 'r', ts: '2026-07-05T10:00:00Z' })] } },
  });
  const costData = path.join(root, 'generated-docs', 'reports', 'build-cost-data.json');
  fs.mkdirSync(path.dirname(costData), { recursive: true });
  fs.writeFileSync(costData, JSON.stringify({ postDelivery: { sessions: [{ id: 'reporting', firstCommand: '/build-report-maintainer' }] } }));

  const { data } = run(root, [`--transcripts=${store}`]);
  assertDeepEqual(data.postDeliveryExcluded, ['reporting'], 'the flagged session is named in the output');
  close(data.totals.totalCost, LINE_COST, 'the report run is not in the total');
  close(data.totals.overheadCost, 0, 'so it cannot masquerade as build scaffolding');
  close(data.benchmarks.costUplift, 1, 'and the fully-loaded uplift stays at the measured 1x');
});

tmp('with no cost data alongside, nothing is auto-excluded', (root) => {
  writeProject(root, { billing: { stories: story(1, 'invoice-table', 0, 30) } });
  const store = writeTranscripts(root, {
    build: { main: [], subagents: { developer: [usageLine({ id: 'b', ts: '2026-07-01T09:05:00Z' })] } },
  });
  const { data } = run(root, [`--transcripts=${store}`]);
  assertDeepEqual(data.postDeliveryExcluded, [], 'run standalone, the generator excludes nothing on its own');
  close(data.totals.totalCost, LINE_COST, 'and the totals are unchanged');
});
...
// Exit 3 = "no project yet, stop"; any other non-zero = "an input is missing, carry on". The
// procedure branches on the code, so the two cases must not share one. They used to both exit 1,
// leaving the orchestrator matching on error prose that had already drifted between the two
// generators ("No epics at" here vs "No epics found at" in the cost generator).
tmp('no epics directory exits 3 — the "stop, there is no project" code', (root) => {
  fs.mkdirSync(path.join(root, 'generated-docs'), { recursive: true });
  const { status, stderr, data } = run(root);
  assertEqual(status, 3, 'exit code');
  assert(/No epics found at/i.test(stderr), 'same wording the cost generator uses for this state');
  assertEqual(data, null, 'nothing written');
});

tmp('epics with no timestamped stories exit 1, not 3 — the report still renders', (root) => {
  writeProject(root, { billing: { stories: { 1: { startedAt: null, completedAt: null } } } });
  const { status, stderr } = run(root);
  assertEqual(status, 1, 'exit code — a missing input, not a missing project');
...
tmp('--rate adds ZAR alongside USD without changing the USD figures', (root) => {
  ...
  const { data } = run(root, [`--transcripts=${store}`, '--rate=18.5']);
  // The renderer reads this to show ZAR alongside USD in the maintainer report's effort section,
  // matching the currency the cost panel above it leads with (renderEffort in
  // generate-build-report-html.js; asserted in its suite).
  assertEqual(data.rate, 18.5, 'rate recorded for the renderer to convert with');
...
tmp('the ambiguous share and the parallel features are both reported', (root) => {
  ...
  assert(/features built in parallel: alpha, beta/.test(stderr), 'and stderr names them');
  // The banner that renders this is asserted in generate-build-report-html.tests.js — it lives
  // on the maintainer page now, so naming it here would test a string this file no longer emits.
});
...
tmp('overlap inside ONE feature never claims features were built in parallel', (root) => {
  ...
  assert(!/parallel: \)/.test(stderr) && !/parallel: ,/.test(stderr), 'never an empty list of features');
  assert(/within a single feature/.test(stderr), 'stderr names the real cause instead');
  // The page's half of this rule — that it never prints "0 of N features … ()" — is asserted
  // against renderEffort in generate-build-report-html.tests.js.
});
...
tmp('the typical-feature story count is a whole number even with an even number of features', (root) => {
  ...
  const { data, stdout } = run(root, [`--transcripts=${store}`]);
  ...
  assert(!/1\.5 stories/.test(stdout), 'no fractional story count in the console summary');
});

// An epic enters `epics[]` on the strength of ONE timed story, so `stories` is a coverage count,
// not the feature's size. Without `storiesTotal`/`timeComplete` the page had no way to tell a
// measured feature from an eight-story feature measured through two of them — and the maintainer
// page used mere presence to decide the epic's time was accounted for, dropping its
// commit-clustered bar and erasing the untimed stories' time from every section.
tmp('an epic reports how many of its stories were timed, not just the timed ones', (root) => {
  writeProject(root, {
    alpha: {
      name: 'Alpha',
      stories: {
        ...story(1, 'invoice-table', 0, 20),
        // Declared but never timed — still part of the feature.
        2: { title: 'approve-invoice', startedAt: null, completedAt: null },
        3: { title: 'export-csv', startedAt: '2026-07-01T09:30:00Z', completedAt: null },
      },
    },
  });
  const store = writeTranscripts(root, { s1: { main: [], subagents: { developer: [usageLine({ id: 'm1', ts: '2026-07-01T09:10:00Z' })] } } });
  const { data } = run(root, [`--transcripts=${store}`]);
  const e = data.epics.find((x) => x.slug === 'alpha');
  assertEqual(e.stories, 1, 'one story had usable timestamps');
  assertEqual(e.storiesTotal, 3, 'the feature declares three');
  assertEqual(e.timeComplete, false, 'so its figures cover part of the feature');
});

tmp('an epic whose every story is timed is marked complete', (root) => {
  writeProject(root, { alpha: { name: 'Alpha', stories: { ...story(1, 'invoice-table', 0, 20), ...story(2, 'export-csv', 30, 50) } } });
  const store = writeTranscripts(root, { s1: { main: [], subagents: { developer: [usageLine({ id: 'm1', ts: '2026-07-01T09:10:00Z' })] } } });
  const { data } = run(root, [`--transcripts=${store}`]);
  const e = data.epics.find((x) => x.slug === 'alpha');
  assertEqual(e.storiesTotal, 2, 'both stories declared');
  assertEqual(e.timeComplete, true, 'and both timed');
});

// The typical-feature median is what the sizing calculator quotes. A partially timed epic's
// buildMinutes/marginalCost cover a subset of its stories, so folding it in drags that median
// below any real feature — an under-quote presented with the same confidence as a measurement.
tmp('the typical-feature benchmark ignores partially timed features', (root) => {
  writeProject(root, {
    whole: { name: 'Whole', stories: { ...story(1, 'invoice-table', 0, 60), ...story(2, 'export-csv', 60, 120) } },
    partial: {
      name: 'Partial',
      stories: {
        ...story(1, 'payment-table', 130, 140),
        2: { title: 'approve-payment', startedAt: null, completedAt: null },
        3: { title: 'reject-payment', startedAt: null, completedAt: null },
      },
    },
  });
  const store = writeTranscripts(root, { s1: { main: [], subagents: { developer: [usageLine({ id: 'm1', ts: '2026-07-01T09:30:00Z' })] } } });
  const { data } = run(root, [`--transcripts=${store}`]);
  assertEqual(data.totals.epics, 2, 'both features are still reported');
  assertEqual(data.benchmarks.epicsMeasured, 1, 'but only the fully timed one backs the benchmark');
  assertEqual(data.benchmarks.partialEpics, 1, 'and the excluded one is counted');
  assertEqual(data.benchmarks.typicalEpic.buildMinutes, 120, "Whole's 120m, not a median dragged to 65m");
  assertEqual(data.benchmarks.typicalEpic.stories, 2, 'and its story count');
});

// Mid-build there may be no fully timed feature at all. A rough benchmark beats none, so it falls
// back to every epic — but epicsMeasured must still say how thin the sample is, and the story
// count must be the features' real size rather than the timed subset.
tmp('with no fully timed feature the benchmark falls back and says how thin it is', (root) => {
  writeProject(root, {
    alpha: {
      name: 'Alpha',
      stories: { ...story(1, 'invoice-table', 0, 30), 2: { title: 'export-csv', startedAt: null, completedAt: null } },
    },
  });
  const store = writeTranscripts(root, { s1: { main: [], subagents: { developer: [usageLine({ id: 'm1', ts: '2026-07-01T09:10:00Z' })] } } });
  const { data } = run(root, [`--transcripts=${store}`]);
  assertEqual(data.benchmarks.epicsMeasured, 1, 'the one partial feature, used as a fallback');
  assertEqual(data.benchmarks.partialEpics, 1, 'reported as partial');
  assertEqual(data.benchmarks.typicalEpic.stories, 2, 'story count is the feature size, not the timed subset');
});
