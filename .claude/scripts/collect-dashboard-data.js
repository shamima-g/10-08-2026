...
// summariseStories / the terminal-phase set live there too, so the hooks' injected
// context and this page can't disagree about story counts or what "finished" means.
const {
  EPIC_PHASES,
  EMPTY_STORY_SUMMARY,
  summariseStories,
...
// Story-summary shape for epics with no readable stories (the invalid-slug /
// missing-state entries below). Same constant summariseStories returns on empty
// input, from lib/epic-state.js.
const EMPTY_STORIES = EMPTY_STORY_SUMMARY;
...
// TERMINAL_PHASES / isTerminalPhase (imported above) name the two phases at which
// an epic's work is finished: COMPLETE-ON-BRANCH (done on its branch — merged or
// ready to merge) and COMPLETE (the after-merge stamp on `main`). The merged-epic
// loop below is where they mark an epic done.
