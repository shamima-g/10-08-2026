 ...
 * the cost and effort data generators.
 *
...
async function main() {
  ...
  // The workflow makes worktrees as `../<project>-plan-<epicSlug>`, so the sibling transcript dir
  // is `<slug>-<suffix>` with a SINGLE dash — requiring `--` matched no real worktree and dropped
  // those epics silently. But extending the slug does not by itself mean "same project":
  // `stadium-8`, `stadium-8-QA` and `stadium-8-upsidedown` are three separate checkouts, and
  // taking every single-dash match folded whole unrelated projects into the totals. Since that
  // spend sits outside every story window it inflates costUplift, which multiplies the
  // "fully loaded" figures and the sizing calculator — a ratio someone quotes a client.
  //
  // `projectRoot` here is not a real repo, so `git worktree list` can't answer and the matcher
  // falls back to the documented `-plan-` shape. This pins both halves: the worktree is kept, the
  // same-prefix stranger is not.
  tmp('discoverTranscriptDirs(): keeps plan worktrees, rejects same-prefix strangers', (root) => {
    const projectRoot = '/c/work/my-app';
    const slug = projectRoot.replace(/[^a-zA-Z0-9]/g, '-');
    const store = path.join(root, 'projects');
    for (const d of [slug, `${slug}-plan-epic-a`, `${slug}-QA`, 'some-other-project', `${slug}0`]) {
    ...
    assertDeepEqual(
      names,
      [slug, `${slug}-plan-epic-a`].sort(),
      'primary + the plan worktree; the `-QA` sibling project, the unrelated project and the same-prefix `<slug>0` all excluded'
    );
  });

  // The authoritative path: inside a real repo, git names every checkout of it, so a listed
  // sibling is proof of membership and an unlisted one is a different project whatever it is
  // named. This is what stops the `-QA`/`-copy`/`-upsidedown` case above needing a name rule.
  tmp('discoverTranscriptDirs(): a real git worktree is included whatever its suffix', (root) => {
    const projectRoot = path.join(root, 'app');
    const worktree = path.join(root, 'app-anything-at-all');
    ...
    git(projectRoot, 'worktree', 'add', '-b', 'wt', worktree, 'main');

    const slug = projectRoot.replace(/[^a-zA-Z0-9]/g, '-');
    const wtSlug = worktree.replace(/[^a-zA-Z0-9]/g, '-');
    const store = path.join(root, 'projects');
    // A stranger sharing the prefix, created alongside, to show git decides and the name doesn't.
    for (const d of [slug, wtSlug, `${slug}-not-a-worktree`]) fs.mkdirSync(path.join(store, d), { recursive: true });

    const { dirs } = discoverTranscriptDirs(projectRoot, store);
    assertDeepEqual(
      dirs.map((d) => path.basename(d)).sort(),
      [slug, wtSlug].sort(),
      'the real worktree is in despite an undocumented suffix; the same-prefix stranger is out'
    );
  });
