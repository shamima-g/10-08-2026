...
**One scoped exception** to "never edited from an `epic/*` branch": a **design-update epic** may carry its own §Styling change on its branch, so updated colours land atomically with the rebuilt screens and the updated digest — see the [§6.2 design-source carve-out](#62-shared-evolving-artifacts--pr-time-conflict-resolution). Every other project-level change still follows the main-landing rule above.
...
|---|---|---|---|
| **Additive union** | Different sections of `globals.css`, different handlers in `handlers.ts`, different API paths in `api-spec.yaml`, additions to barrel `index.ts`, new epic rows in `epic-plan.md`, new bullets in the design digest's `Your Decisions` section | Auto-merge keeping both sides | Tier 1 (autonomous) |
...
### Design source — the one project-level artifact a design-update epic edits on its branch

The **design source** — the design files the user placed in `documentation/` (any shape) **and** the interpreter's `generated-docs/design/digest.md` — is project-level and normally lives on `main`. There is **one deliberate exception**: a **design-update epic** (a normal epic the user creates via `/start` to rebuild against a changed design — [start.md Step B2](../commands/start.md#step-b2-cut-the-epic-branch-fresh-from-main); not `/plan`, which can't carry the design source atomically) updates the design files **and refreshes the digest on its own branch**, and the change reaches `main` only when that epic merges — together with the rebuilt screens that consume it. **When the update also changes the brand palette, the same branch carries the `project.md` §Styling update and the regenerated `web/src/app/globals.css`** — so colours, screens, and the digest land as one atomic unit.

This is safe, and it's *more* correct than putting the design (or the palette) on `main` up front:

- **No other epic rewrites the design source.** Normal feature epics never edit `documentation/`'s design files or the digest's design content — their only write is an append to the digest's `Your Decisions` section ([approval-pattern.md](../shared/approval-pattern.md#recording-a-decision-that-overrides-the-design)), which merges as a **Tier 1 additive union**. So a design-update epic can't produce the cross-epic conflicts the table above guards against.
- **`main`'s design always equals what's built.** The design (and its digest) only land when the rebuild lands — no separate "last-built" bookkeeping, and git history holds prior versions.
- **The whole update is atomic.** Merge applies the new design files, updated digest, screens, **and** palette together; **abandoning the epic (`git branch -D`) discards all of it** — including the colour change — with nothing half-applied on `main`. (This is why it all rides the branch rather than landing on `main` up front: a mid-flight abandon must not leave `main` re-branded/re-digested but un-rebuilt.)
- **Two concurrent design-update epics** edit the design source (and §Styling) from the same baseline, so the second to merge hits a normal **Tier 4 conflict**, resolved by the user. The user picks when to do design work, so this is rare.

**Scope of the `project.md` exception:** *only* a design-update epic, and *only* its §Styling section, may be edited on a branch — because the palette is intrinsic to the design being rebuilt. **Every other `project.md` section (roles, auth, data source, compliance) still follows §6.1** — main-only, never on an epic branch. `epic-plan.md` likewise stays main-only. A **colours-only update** (no screens) is scoped by the user as a styling epic and rides its branch the same way, or — when the user prefers — applied to `main` directly as a §6.1 project change; either way there is no separate refresh machinery.
