...
| **Workflow refinements** | Brief lists 5 steps, design shows 4 + an implicit step — record the actual sequence. |
...
**`requiresProjectChange` routing.** When the halt proposes a change to project-level facts (`project.md` — roles, auth, data source, compliance, styling, design source), set `requiresProjectChange: true`. The orchestrator then resolves it inline per [epic-branch-concurrency.md §6.1](../policies/epic-branch-concurrency.md#§61-project-level-changes) rather than surfacing the halt to the user.
