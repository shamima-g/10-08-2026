...
**Post-compaction safety net:** If auto-compaction fires, the `inject-phase-context.js` hook automatically restores workflow state via `additionalContext`. The hook resolves the active state.json via `resolve-state-path.js` and loads the matching `.claude/hooks/phase-context/<phase>.md` (`plan.md`, `build.md`, `epic-end.md`). This covers both orchestrator and subagent sessions.
