...
- **Metrics, timeline, build flow (story swimlanes & parallelism), cost & user involvement,
  workflow performance, effort benchmarks (per-screen-type medians, per-feature roll-up and the
  "size a new feature" calculator), quality-gate history, codebase stats, per-epic breakdown,
  stumbling blocks, data quality** — computed deterministically from git history, the workflow's
  own files (`state.json`, `journal.md`, `template-feedback.md`, tracked files under `web/`), the
  session-log cost and effort data, and the quality-gate run log. You don't write these; the
  generator does.
...
Extract `ZAR` and pass it as `--rate` to **both** generators in the next step. If the fetch fails,
proceed without `--rate` — the report falls back to a placeholder rate and prints a note on the
page saying the rand figures rest on it. Say the same thing in your spoken summary.
...
delivered, the first-pass E2E yield, the typical cost of a feature from the effort benchmarks,
and the single biggest time-sink. Mention any excluded sessions in one clause.

Point out that the page carries a **sizing calculator** — the reader enters the screen mix for the
next feature and reads off expected build time and cost. If the effort data came back
`costComplete: false`, say the benchmarks are time-only and why, and quote no per-story cost.

Mention the **Spend detail** fold on the page for anyone who wants to go deeper — per-model,
per-tool and per-sub-agent breakdowns, and the verbatim log of every question with the answer
given. It's collapsed by default, so say it's there.
