...
Restart Claude Code before you carry on building. This session is still using the
old versions of the workflow's background checks, so it may show errors on each
message until you do.
```

Ask for the restart whenever the upgrade touched anything under `.claude/hooks/` or the `hooks`
section of `.claude/settings.json` — Claude Code reads those once when a session starts, so a
renamed or removed file leaves this session pointing at something that's no longer there. Say the
first line either way; add the restart lines only when it applies.
