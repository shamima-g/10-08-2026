...
**Only if the open command succeeded** (exit 0). Display:
...
If it exited non-zero, it could not open a browser on this computer (headless Linux, a
container, or a stripped-down WSL install) and it printed the file's full path. Don't say the
dashboard is open — pass that path on so the user can open it themselves:

```
I couldn't open a browser on this machine. Open this file to see the dashboard:
<the path the script printed>
```
