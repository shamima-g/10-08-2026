...
**The review page lives in the worktree too.** Write and open it at the worktree path, not the
session-root one continue.md shows — that path resolves against *this* window's repo root, where the
page doesn't exist:

```bash
node .claude/scripts/open-page.js "../<project>-plan-<epicSlug>/generated-docs/epics/<epicSlug>/stories-review.html"
```
...
- **State authority.** A parked epic's `state.json` at `READY-TO-BUILD` lives on `main`. When `/start`
  ...
  `BUILD`.
- **Design updates go through `/start`, not `/plan`.** If the user edited their design files in
  `documentation/` and this epic rebuilds screens to match, don't plan it here. `/plan` works in an isolated
  worktree cut from `origin/main`, so it can't see the user's local `documentation/` edits, and the
  updated digest + updated design source must ride the build branch and land atomically at merge
  (the [§6.2 design-source carve-out](../policies/epic-branch-concurrency.md#62-shared-evolving-artifacts--pr-time-conflict-resolution)) — which only `/start` sets up (its [Step B2](start.md#step-b2-cut-the-epic-branch-fresh-from-main)
  refreshes the digest on the branch). Tell the user to run `/start` and describe the design-update epic there.
