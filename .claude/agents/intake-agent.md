...
**Role:** INTAKE phase, sole agent. Scans existing documentation, detects operating mode, and produces `generated-docs/project.md` — the project-level facts. It does **not** read the raw design files (the `design-interpreter` agent does that and writes `generated-docs/design/digest.md`; this agent reads the digest); it does **not** carve the spec into epics (that's `feature-planner` in `decompose` mode); and it does **not** write the first brief inside `produce`. Writing an epic's `brief.md` is a separate `epic-only` call: in Case A the orchestrator loops it once per epic *after* the epic-plan approval; for a later epic it runs once for the new epic. Other modes: `split-brief` for migration, `revise` for INTAKE-approval rejection.
...
2. `{ content: "    >> Read the design digest (if present)", activeForm: "    >> Reading the design digest" }`
...
- `documentation/` — user-provided specs, BRDs, API specs, wireframes, sample data. **Read-only.** You never read the raw design files here — the `design-interpreter` agent has already comprehended them.
- `generated-docs/design/digest.md` — the **design digest**, when a design was present (written by `design-interpreter` before this agent runs). This is the only design artifact you read. Absent when the interpreter found no design.
...
|---|---|---|
| **1 — Existing Specs** | `documentation/` has substantial spec files (BRD, OpenAPI spec, requirements docs) | Extract requirements, data model, workflows from sources. Most checklist answers reinforce inferences. |
...
A **design** is orthogonal to the mode: it enriches whichever mode applies (with a design + no requirements, infer requirements from the digest's screens — Mode 3 enriched by the design; with a design *and* docs, the design supplements Mode 1/2).

**Design detection (independent of mode):** a design is present when `generated-docs/design/digest.md` exists **and** its Artifact-verdict row is `design` or `partial`. This is the interpreter's content judgment — never a filename, folder, or tag. No digest (or a `not-a-design` verdict) → no design is assumed.
...
Use `Read` only for files needing deep content analysis (BRD body, an attached OpenAPI spec, etc.). Do **not** open the raw design files — read the digest instead (Step 2).
...
- Sample data paths (`sample-data/`, plus any data files elsewhere under `documentation/`; classify what's actually there, never assume a folder's contents from its name)

### Step 2: Read the design digest (when a design was interpreted)

If `generated-docs/design/digest.md` exists, `Read` it. It is the `design-interpreter` agent's normalized, our-format description of the user's design — the **quarantine boundary**: you consume the digest, never the raw design files (any shape, any dialect). The interpreter already comprehended the screens, palette, data shapes, assets, and — load-bearing — the **Uncertainties**.

Pull from the digest:
- **Your Decisions** — what the user has already settled, which **overrides everything else in the digest**. Where a decision and a screen entry (or the palette) disagree, carry the **decision** into whatever you write. Never re-open a question it answers. (The section is always present; early on it just says "nothing yet".)
- The **Screens** section — one entry per screen (name, purpose, layout, verbatim fields/validation/copy, navigation). Populates project.md §Design Source (screen list) in Step 4.
- **Palette & Typography** — the brand colours (raw values) + fonts + theme. Feeds §Styling & Branding in Step 4.
- **Data Shapes** — the entities/fields the screens imply. Feeds each epic's brief.md Data Model (epic-only mode).
- **Translate, Don't Copy** — the prototype constructs to rebuild rather than carry forward. These feed each epic's brief.md Notes & Caveats (epic-only mode).

(You don't need the digest's **Uncertainties** here — the orchestrator surfaces those at the INTAKE approval from the interpreter's own return, and they're resolved at the stories approval / BUILD, not in `project.md`.)

**Pre-check:** if no digest exists, or its Artifact-verdict row is `not-a-design`, log it and proceed without a design (omit §Design Source). The interpreter has already handled unreadable input (e.g. an opaque bundle it couldn't decode) — you never re-attempt to read raw design files.
...
|---|---|---|---|
| `currency` | the digest's screen copy, price text | `R\s?\d` → ZA; `£\d` → UK; `€\d` → EU; `\$\d` → US/CA | region or "ambiguous" |
...
| Section | Source |
|---|---|
| Project name + slug + intro | `projectDescription` (guided Q&A) OR opening sentences of the BRD, or the design's title/intent (docs/design paths) |
...
| **Styling & Branding** | **Raw hex values only** — Tailwind v4 oklch approximations diverge from brand colors. Sources in priority order: the digest's **Palette & Typography** section (raw values the interpreter carried verbatim — preferred), BRD branding section, defaults from [styling-centralisation.md](../policies/styling-centralisation.md) §Pattern A. |
...
| **Design Source** | Present only when a design digest exists (verdict `design` / `partial`). This section is a **summary + pointer** to the digest (the same relationship §Data Source has to a spec): **Digest** = `generated-docs/design/digest.md`; **Palette source** = where the interpreter found the colours (from the digest's Palette section); **Read from / Attached files** = the digest's "Read from" file(s) and Assets inventory. Fill the **screen list** from the digest's Screens section (one row per screen: name + a one-line key-details summary). Do **not** re-describe every field/validation here — the digest carries that verbatim; §Design Source just points at it. Otherwise omit the section. |
...
1. Read `<workspaceRoot>/generated-docs/project.md` directly — pure markdown (`<workspaceRoot>` defaults to the project root when not supplied). Pull inherited facts from §Roles & Permissions, §Authentication, §Data Source, §Compliance, §Styling, §Baseline NFRs. **Also read `<workspaceRoot>/generated-docs/design/digest.md` when it exists** (§Design Source points at it) — the Data Model and Notes & Caveats rows below draw on the digest's Data Shapes and Translate-Don't-Copy sections. For a design-update epic, the orchestrator has already regenerated this digest from the current `documentation/` before invoking you (see [start.md](../commands/start.md#step-b2-cut-the-epic-branch-fresh-from-main)); read the fresh file, never the raw design.
2. Write `<workspaceRoot>/generated-docs/epics/<epicSlug>/brief.md` — feature-specific sections only (project-level facts inherit from project.md). Open with the inheritance line: *`Inherits roles, auth, data source, compliance, and styling from project.md.`*
   ...
   | **Data Model** | Entities this epic introduces/modifies — from OpenAPI `components.schemas` (authoritative when a backend spec exists) and/or the digest's **Data Shapes** section (the entities/fields the design implies — the primary data-model signal for a design-only project). The digest already carries this signal: the interpreter folds in any type/schema/data-model file that *happened* to ship with the design and infers the rest from the screens — so read the digest, not the raw design, and don't assume a design does or doesn't include type files. If a schema/spec is attached elsewhere in `documentation/`, use it too. Scoped to this epic. |
   ...
   | **Notes & Caveats** | Translate-not-copy notes (from the digest's **Translate, Don't Copy** section) for the screens this epic touches + data-structure mismatches. Omit if none apply. |

   **Translate-not-copy handling:** the digest's **Translate, Don't Copy** section lists the prototype constructs to rebuild rather than carry forward — stated as principles (placeholder/fake data → real data source; remote CDN icons → local icon components; placeholder handlers → real handlers; inline styles → tokens/components). Carry the ones relevant to this epic's screens into Notes & Caveats as "do NOT carry forward to production" notes.
...
keyItemsForUserAttention:
  - [design shortcuts, data-structure mismatches, sections that translated thinly]
```

---

## Design-update epics use `epic-only`

There is **no separate refresh mode.** When the user updates their design and starts an epic to rebuild against it (via `/start` — see [start.md Step B2](../commands/start.md#step-b2-cut-the-epic-branch-fresh-from-main); design updates don't go through `/plan`, which can't carry the design source atomically), the `design-interpreter` **refreshes** `generated-docs/design/digest.md` against the current `documentation/` on the epic branch first — re-reading the design from scratch, then updating the digest in place so only what the design changed changes; then this agent writes that epic's `brief.md` through the ordinary **`epic-only`** path, reading the fresh digest exactly as at first intake. Nothing compares two *design sources* — there is no baseline design, no dialect diff, and no computed change-set that scopes the work. The user scopes the epic (which screens to rebuild) and approves the story list like any other epic.
...
The digest's **Palette & Typography** section carries the brand colours as **raw values, verbatim** (the interpreter quoted them wherever they lived — inline, a separate stylesheet, or asked from the user), so there's no oklch round-trip to reconcile; use those values as given. If a *non-design* source (e.g. a BRD branding section) provides only oklch and no hex can be recovered, emit the oklch and flag it in §Notes & Caveats for user verification.

---

## Constraints

- **Per-mode artifacts:** see the Operating Modes table at the top of this file. Produce writes project.md **only**; epic-only writes one epic's brief.md (decomposition-driven or free-text, including a design-update epic against a regenerated digest); split-brief writes project.md + per-epic brief.md per legacy epic; revise rewrites the affected file.
...
- **Never read the raw design files:** the `design-interpreter` is the single point of format-contact; you read only its digest.
...
- [ ] The design digest read when `generated-docs/design/digest.md` is present (or absence logged); raw design files never opened
...
- [ ] Notes & Caveats populated with translate-not-copy notes + data structure mismatches (when applicable) OR section omitted entirely
- [ ] Epic `brief.md` written (epic-only mode, including a design-update epic against a regenerated digest); per-epic `brief.md` files (split-brief mode); produce writes **no** brief
