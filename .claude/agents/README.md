...
|-------|-------|-------------|
| **INTAKE** | design-interpreter | The sole reader of raw design files: read whatever the user dropped in `documentation/` (any shape), comprehend it semantically, and emit the normalized design digest (`generated-docs/design/digest.md`) — the single point of format-contact every downstream agent reads instead of the raw design. On a later design update it re-reads from scratch but **reconciles** the digest in place, changing only what the design changed |
...
**Purpose:** Implements exactly one story at a time. Reads `project.md` + the epic's `brief.md` as the source of truth, applies the agent autonomy policy, and halts only on always-halt conditions. When a design digest (`generated-docs/design/digest.md`) is present, it rebuilds the story's screen faithfully in this stack from the digest's screen entry rather than any source markup. Makes the failing tests pass using App Router, Shadcn UI, and the project's API client.
