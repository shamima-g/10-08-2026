...
|---|---|---|---|
| `decompose` | `/start` Case A (full spec up front) | `project.md` + the spec in `documentation/` (via `scanResult`) + the design digest (`generated-docs/design/digest.md`) when present | `DECOMPOSITION PROPOSAL` — see [Decompose Mode](#decompose-mode-up-front-epic-planning) |
...
- `generated-docs/design/digest.md` — when present, the design digest's screen inventory (also summarized in `project.md` §Design Source) informs story sizing
...
### Step 2: Codebase + design-source scan (~1–2 min)

- `ls web/src/` one level deep — what already exists from the template and prior epics
- If `generated-docs/design/digest.md` exists, read its **Screens** section (also summarized in `project.md` §Design Source) for the screen list — the design's screen inventory is a strong signal for story sizing — **plus its Your Decisions and Uncertainties sections** (see [Design Choices](#design-choices-from-open-design-uncertainties) below). Your Decisions **overrides** anything a screen entry says.
...
designScreens:
  - "<screen name>"
  - ...   # only when generated-docs/design/digest.md exists; the digest screen names this epic's stories build. The developer agent uses them to look up each screen's entry in the digest during design-source enforcement.

designChoices:            # 0–N; [] is common and the default when there's no design digest. See § Design Choices. Each an either/or the user should own, sourced from open digest Uncertainties on this epic's screens. The orchestrator ADDS these to the stories-review page's existing "Design choices" section (it still derives any plan-level either/ors itself, unchanged).
  - question: "<plain-language decision the user owns — e.g. 'Where should the Dashboard \"Export\" button lead?'>"
    options: ["<recommended option first>", "<alternative>"]   # orchestrator adds a free-text \"Something else\"
    recommended: "<the option you'd default to>"
    screen: "<the digest screen this concerns>"
  - ...
```
...
1. **Extract the requirement inventory.** From the spec sources — the BRD / OpenAPI in `documentation/`, plus `generated-docs/design/digest.md` when it exists (`project.md` §Design Source points at it; its screens/data-shapes are a requirement source) — build one flat, numbered list of atomic requirements — `R1..RN` across the **whole** spec. These are *provisional, plan-level* IDs (each epic's `brief.md` re-numbers locally later, the same way today's briefs do). Each entry: `id`, a plain-language `name` ([Translation Rule](#translation-rule)), and the source `text`. This list is the coverage denominator.
...
## Design Choices (from open design uncertainties)

`designChoices` feeds the stories-review page's existing **"Design choices"** section (per [approval-pattern.md § Design choices](../shared/approval-pattern.md#editable-html-review-page-plan-approvals)) — decisions the user owns, surfaced *before* the epic is built and folded into the developer's per-story context on approval. **This field is one source into that section: the open Uncertainties in the design digest.** The orchestrator still derives any plan-level either/ors itself (unchanged); when there's no digest, this field is simply `[]` and nothing about the existing behaviour changes.

**Why surface them here.** A design uncertainty ("Where should the 'Export' button lead?", "Two screens both say 'Details' — are they distinct?") is a question the user *already knows the answer to* and the developer needs *to build correctly*. Asking it at PLAN resolves it before any build; deferring it would mean building on a guess and rebuilding after the user corrects it. (Contrast `unverifiedAssumptions` above: those are *backend-reality* checks the user can only confirm against the running app — a different question at a different stage.)

**How to build the list.** For each open Uncertainty in the digest that touches a screen this epic builds:

- **Skip it if the digest's Your Decisions section already answers it.** That section records what the user settled at an earlier approval and **outranks the screen entries** (newest entry on a topic wins). An Uncertainty it answers is **not** open: apply the recorded decision as the story's default and don't re-ask. Same for a decision that contradicts a screen entry — plan to the decision, not the design.
- **Apply the same inclusion test as any design choice** — omit it if you're **≥80% sure** of the right answer (just pick it, apply it as the story's default, and don't ask). A digest Uncertainty is by definition the interpreter's *couldn't-determine* (<80%) case, so most surface here.
- Phrase the `question` in plain language ([Translation Rule](#translation-rule)); give `options` with the **recommended one first** (the orchestrator adds a free-text "Something else"); set `screen` to the digest screen it concerns.
- Uncertainties that are **structurally blocking with no reasonable default** aren't a radio choice — leave them in the digest; the developer will HALT on them at BUILD (developer.md Step 2). Anything non-blocking with a sensible default that you *don't* surface here still gets caught at BUILD as an `unverifiedAssumption` (the backstop) — but surfacing it here is the better experience.

**Keep it tight: 0–N, `[]` is common.** Only genuine either/ors. Don't manufacture a choice from an uncertainty you can reasonably default.
...
- [ ] `designScreens` populated when `generated-docs/design/digest.md` exists for the epic's screens
- [ ] `designChoices` surfaces open digest **Uncertainties** on this epic's screens as user-owned decisions (recommended option first; `[]` when none, or when all are ≥80%-defaultable or structurally-blocking — the latter left for the developer to HALT on)
