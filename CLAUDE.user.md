...
/build-report-maintainer   # Open a visual build report — effort, cost, workflow performance, what each kind of screen costs to build (with a calculator for sizing the next piece of work), what was built, where time was lost
...
**Development work goes through the TDD workflow.** When the user asks you to build, create, add, change, fix, or implement anything — including casual phrasings like "make it look nice" or "tweak the header" — follow the `Action:` line the `workflow-guard.js` hook injects at the top of every prompt (typically a redirect to `/start` or `/continue`).
